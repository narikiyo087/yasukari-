...
> = {
  ...
  full_en: {
    label: "本登録（英語版）",
    subject: "[Yasukari] Full registration completed",
    category: "本登録",
  },
  reservation_adachi: {
    label: "予約受付完了（足立小台店）",
  ...
  reservation_adachi_en: {
    label: "予約受付完了（足立小台店・英語版）",
    subject: "[Yasukari] Your bike rental reservation is confirmed",
    category: "予約完了",
  },
  reservation_minowa_en: {
    label: "予約受付完了（三ノ輪店・英語版）",
    subject: "[Yasukari] Your bike rental reservation is confirmed",
    category: "予約完了",
  },
  extension: {
...
const buildSampleReservation = (
  email: string,
  storeName: "足立小台店" | "三ノ輪店",
  useEnglish: boolean
): Reservation => {
  const now = new Date();
  ...
  return {
    ...
    notes: useEnglish ? "Saved via Pay.JP payment (test data)." : "テスト送信用のサンプルデータです。",
    refundNote: "",
  };
};
...
export default async function handler(
  ...
  try {
    ...
    if (selectedType === "full_en") {
      ...
      await deliverFullRegistrationEmail(normalizedEmail, "en");
      res.status(200).json({ message: "本登録（英語版）のテストメールを送信しました。", status: "sent" });
      return;
    }
    ...
    const sampleReservation =
      selectedType === "reservation_minowa" || selectedType === "reservation_minowa_en"
        ...
        : buildSampleReservation(normalizedEmail, "足立小台店", isEnglishReservation);
