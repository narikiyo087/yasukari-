...
export default function ReserveTermsAgreement() {
  ...
  useEffect(() => {
    ...
    void router.replace(`/reserve/flow/step2?${nextSearch}`);
  ...
  const handleAgree = async () => {
    ...
    try {
      ...
      if (response.status === 404) {
        throw new Error(payload.message ?? '本登録情報が見つかりません。先に本登録を完了してください。');
      }

      if (!response.ok) {
        throw new Error(payload.message ?? '利用規約への同意を保存できませんでした。');
      }
      ...
      void router.push(`/reserve/flow/step2${search}`);
    } catch (error) {
      console.error(error);
      setAgreeError(error instanceof Error ? error.message : '利用規約への同意を保存できませんでした。');
    } finally {
      setAgreeing(false);
    }
  };
