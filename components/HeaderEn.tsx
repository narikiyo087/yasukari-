          ...
          <div className="flex items-center gap-4">
            ...
            <nav className="hidden sm:flex items-center gap-6 text-sm font-medium">
              ...
              <Link href="/en/notifications" aria-label="Notifications">
                <span className="flex items-center text-slate-700 hover:text-red-600 transition-colors">
...
function NavItem({ icon, label }: { icon?: React.ReactNode; label: string }) {
  return (
    <button className="flex items-center gap-1 text-slate-700 hover:text-red-600 transition-colors">
      {icon && <span>{icon}</span>}
      <span>{label}</span>
    </button>
  );
